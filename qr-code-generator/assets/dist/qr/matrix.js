import { evaluate_matrix } from "./qr.js";
import { alignment_pattern, alignment_pattern_location_table, ec_bits_table, finder_pattern, format_information_table, mask_pattern, version_information_binary } from "./qr_table.js";
export class Module {
    x;
    y;
    value;
    location;
    constructor(x, y, value, location) {
        this.x = x;
        this.y = y;
        this.value = value;
...
export class Matrix {
    ...
    draw_alignment_pattern() {
        ...
        for (let i = 0; i < locations.length; i++) {
            alignment_loop: for (let j = 0; j < locations.length; j++) {
                //
    ...
    insert_codewords(codewords, mask = () => false) {
        ...
        for (let column = temp_grid.length - 1; column >= 0; column -= 2) {
            ...
            const condition = (row) => is_upwards ? row >= end : row <= end;
    ...
    add_format_information(grid, mask) {
     ...
     */
    add_data(codewords) {
        let best_grid = this.insert_codewords(codewords, mask_pattern[0]);
